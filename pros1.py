...
class SimpleVectorStore:
    ...
    def remove_file(self, filename: str) -> bool:
        """从索引中删除指定文件的所有分块，并重建索引"""
        if not self.documents:
            return False
        # 过滤掉 source 为 filename 的文档
...
class EnhancedKnowledgeBase:
    ...
    def delete_file(self, filename: str):
        ...
        if remaining_files:
            with open(self.vector_store.metadata_file, 'wb') as f:
                pickle.dump({"files_info": remaining_files, "updated": datetime.now().isoformat()}, f)
        else:
            # 无文件时确保 metadata 也被清除
            if os.path.exists(self.vector_store.metadata_file):
                os.remove(self.vector_store.metadata_file)
        # 重新加载索引状态
    ...
    def _kb_panel(self):
        ...
        if files:
            for f in files:
                ...
                # # 为每个文档维护一个独立的 popover 打开状态，为解决删除一个文档后自动弹出下一个文档确认框的问题
                # popover_state_key = f"popover_{delete_key}"
                # if popover_state_key not in st.session_state:
                    ...
                    with col3.popover("删除", use_container_width=False):
                        ...
                        with col_confirm:
                            if st.button("确认删除", key=f"confirm_{delete_key}", type="primary"):
                                ...
                                st.rerun()
                        # 取消按钮始终无法自动关闭二次确认dialog，先注释不显示取消
                        # with col_cancel:
